...

{
	...
	COLORREF Col;                                    // Шаг по азимуту,градусы
	double df = 0.4; double dq = 0.4; double kLight; // Градусы	
...
//------ Проходим по точкам сферы  ---------------------------------------
	for (double fi = 0; fi <= 180.0; fi += df)  // Азимут
		for (double q = 0; q <= 360.0; q += dq) // q= 90 - угол места
		{
			...
			PV(0) = VCart(0); PV(1) = VCart(1); PV(2) = VCart(2); PV(3) = 1; PV = MV*PV;
			VCart(0) = PV(0); VCart(1) = PV(1); VCart(2) = 1; VCart = MW*VCart;  // Координаты точки в видовой системе координат
			CMatrix  VP = VS - VR;
			double cos_PN = CosV1V2(VP, VSphereNorm); // Направление на источник света относительно
                                       // нормали к точке падения	
			if (cos_PN > 0) // Если точка сферы освещается...
			{
				if (Index == 0)//Диффузионная модель отражения света 
					kLight = cos_PN; // угла света
				...
				Col = RGB(kLight*133 , kLight*255, kLight*255);
