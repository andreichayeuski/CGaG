...
{
	double dx = 0.25, dy = 0.25;	//шаг функции
	r = 50, fi = 30, q = 45;		//радиус и азимуты, т.е. точка просмотра
	CRectD SpaceRect(-20, 20, 20, -20);	//задаем область область прорисовки
	CRect  WinRect;
	...
	Graph1.SetWinRect(WinRect);		//получаем рабочую область окна
	Index = 1;
	Invalidate();
}
...
{
	double dx = 0.25, dy = 0.25; // шг ф-и
	r = 50, fi = 30, q = 45;             //радиус и азимуты, т.е. точка просмотра
	CRectD SpaceRect(-15, 15, 15, -15);  //задаем область область прорисовки
	CRect  WinRect;
	GetClientRect(WinRect);              //получаем рабочую область
	WinRect.SetRect(WinRect.left + 50, WinRect.top + 50, WinRect.right - 50, WinRect.bottom - 50); //задаем рабочую область
...
void CMainWnd::OnCplot3dFunc2()
{
	...
	CRectD SpaceRect(-1, 1.05, 1.05, -1);
	CRect  WinRect;
